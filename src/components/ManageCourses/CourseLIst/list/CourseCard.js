  ...
  const renderGrid = () => (
    <Row className="mt-2">
      {item.map((item, index) => {
        ...
        let persianDate = "نامشخص";
        try {
          persianDate = new PersianDate(new Date(item?.lastUpdate)).format("YYYY/MM/DD");
        } catch (error) {
          console.error("خطا در تبدیل تاریخ:", error);
        }

        return (
          <Col key={index} md="3" sm="6" xs="12" className="d-flex justify-content-center align-items-center">
                  ...
                  <div>
                    ...
                    <div className="d-flex justify-content-between align-items-center mt-1">
                      <div className="d-flex align-items-center">
                        ...
                        <h6 className="mb-0 ml-1" style={{ marginRight: "6px" }} dir="rtl">
                          {persianDate}
                        </h6>
                      </div>
                      <Badge
        ...
        {item.map((item, index) => {
          ...
          return (
            <tr key={index}>
              ...
              <td>
                <Link className="text-primary" to={`/courses/${item.courseId}`}>
                  {item.title}
                </Link>
              </td>
              <td>{item.fullName}</td>
              <td>{item.reserveCount}</td>
              <td dir="rtl">{persianDate}</td>
              <td>
                <Badge
                  pill
                  color={
