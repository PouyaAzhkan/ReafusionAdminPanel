...
const HomeWorksModal = ({ open, handleModal, sessionId }) => {
  ...
  const [openAddStudentHomeWork, setOpenAddStudentHomeWork] = useState({ open: false, hwid: null });
  const rowsPerPage = 6;
  ...
  const handleAddStudentHomeWorkModal = () => {
    setOpenAddStudentHomeWork({ open: false, hwid: null });
  };
  ...
  return (
    <Fragment>
      <Modal
        isOpen={open}
        toggle={handleModal} // استفاده از handleModal به‌جای handleCloseModal
      ...
      >
        ...
        <ModalFooter className="justify-content-between">
          <Button
            onClick={handleAddHomeWorkModal}
            type="submit"
            className="me-1 mt-2"
            color="primary"
          >
          ...
          <Button
            ...
            color="secondary"
            outline
            onClick={handleModal} // استفاده از handleModal
          >
            انصراف
          </Button>
        </ModalFooter>
      ...
      <AddStudentHomeWorkModal
        open={openAddStudentHomeWork.open}
        handleModal={handleAddStudentHomeWorkModal}
        hwid={openAddStudentHomeWork.hwid}
      />
    </Fragment>
  );
};
