...
import PersianDate from "persian-date"; // اضافه کردن کتابخانه
...
const WeblogCard = ({ Api }) => {
  return (
    <>
      {Api?.news?.map((item) => {
        const persianDate = new PersianDate(new Date(item.insertDate)).format("YYYY/MM/DD");
          ...
          <Card key={item.id} className="WeblogCard">
            ...
            <CardFooter className="d-flex justify-content-between gap-1 text-black">
              <div>
                ...
                <small className="ms-25">{persianDate}</small> {/* نمایش تاریخ شمسی */}
              </div>
              <div>
