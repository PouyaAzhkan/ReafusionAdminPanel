export const CourseGroups = ['نام گروه','نام مدرس','ظرفیت','عملیات',]
export const CourseTableTitles = ['نام دانشجو','نام دوره','وضعیت','جزعیات دوره']
export const PaymentCoursesTableTitles = ['نام کاربر','نام دوره','وضعیت پرداخت','تایین وضعیت']
export const technologiesTableTitles = ['','نام تکنولوژی','توضیحات تکنولوژی','اقدام']
export const StatusTableTitles = ['','نام وضعیت','توضیحات وضعیت','اقدام']
export const LevelsTableTitles = ['','نام سطح','توضیحات سطح','اقدام']

export const SocialGroupsTableTitle = ['','نام گروه','لینک گروه','عملیات',]



