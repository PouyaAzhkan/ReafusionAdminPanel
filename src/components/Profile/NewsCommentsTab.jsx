...
import { getAdminNewsComments } from "../../@core/Services/Api/AdminInfo/AdminInfo";
import { ChevronDown, ExternalLink, MoreVertical, Trash2 } from "react-feather";
...
const NewsCommentsTab = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(10);

  const { data, isError, isLoading, refetch } = getAdminNewsComments({ cacheTime: 0 });
  ...
  // فیلتر کردن داده‌ها
  const filteredData = data?.myNewsCommetDtos?.filter((item) =>
  ...
  return (
    <Fragment>
      <Card className="overflow-hidden">
        <CardHeader>
          <CardTitle tag="h4">کامنت های شما در اخبار</CardTitle>
        </CardHeader>
        <CardBody>
          {isLoading && (
...
export default NewsCommentsTab;